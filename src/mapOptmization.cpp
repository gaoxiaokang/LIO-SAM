...
public:
    ...
    mapOptimization()
    {
        ...
        //add 20230417
        pubGps_raw                  = nh.advertise<sensor_msgs::NavSatFix>("gps_serial/gps_pub_raw", 1);         


        subCloud = nh.subscribe<lio_sam::cloud_info>("lio_sam/feature/cloud_info", 1, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        subGPS_raw = nh.subscribe<sensor_msgs::NavSatFix>(gpsTopic_raw, 200, &mapOptimization::gps_rawHandler, this, ros::TransportHints().tcpNoDelay());
    ...
    void gps_rawHandler(const sensor_msgs::NavSatFix::ConstPtr& gps_rawMsg)
    {
        sensor_msgs::NavSatFix gps_raw;
        
        // Extract all members of gps_rawMsg and assign them to gps_raw
        gps_raw.header = gps_rawMsg->header;
        gps_raw.status = gps_rawMsg->status;
        double lon_degree, lon_minute;
        double lat_degree, lat_minute;
        lon_degree = floor(gps_rawMsg->longitude/100);
        lat_degree = floor(gps_rawMsg->latitude/100);

        lon_minute = fmod(gps_rawMsg->longitude,100);
        lat_minute = fmod(gps_rawMsg->latitude,100);


        gps_raw.latitude = lat_degree + lat_minute / 60;
        gps_raw.longitude = lon_degree + lon_minute / 60;
    
        gps_raw.position_covariance = gps_rawMsg->position_covariance;
        gps_raw.position_covariance_type = gps_rawMsg->position_covariance_type;

        pubGps_raw.publish(gps_raw);
   ...
   //added by gxk 0625
    ...
    void updateInitialGuess()
    {
        ...
        {
            ROS_INFO("cloudInfo.imuAvailable == true");
